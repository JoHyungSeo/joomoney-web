            ...
            <div class="flex flex-col fs-14 fw-400 lh-22 gap-4">
              ...
              <div>
                <div class="flex min-w-0">
                  <div class="flex flex-1 flex-nowrap ali-items-center max-w-100vw w-100pct min-w-0 h-48 px-10 py-6 gap-8 fs-16 border-solid-1 border-input br-10 bg-none bg-transparent hover:border-yellow focus-within:border-yellow transition-all transition-dur-250 transition-timing-ease-in-out">
                    <input
...
const errorCodeMap: Record<string, string> = {
  ...
  ERR_EMAIL_VERIFY_COOLDOWN: "auth.error.email.cooldown",
  ERR_EMAIL_SMTP_SEND_FAILED: "auth.error.email.sendFailed",
}
...
const requestPasswordReset = async () => {
  ...
  if (userIdError.value) return

  processing.value = true

  store.resetPassword.setUserId(userId.value)

  const res = await store.auth.passwordResetRequest(userId.value)

  if (res.success) {
    store.resetPassword.setMaskedEmail(res.data.maskedEmail)
    goPage("ResetPasswordVerification")
  } else {
    if (res.error instanceof AxiosError) {
      const resultCode = res.error.response?.data?.code as string
      userIdError.value = errorCodeMap[resultCode] ?? "auth.error.resetPassword.failed"
    }
  }
